...
export function TargetsClient({ managerId }: TargetsClientProps) {
  ...
  const loadTargets = async () => {
    if (!managerId) return;
    setLoading(true);
    const response = await fetch(`/api/targets/bulk?managerId=${managerId}&year=${DEFAULT_YEAR}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Hedef Girişi</h1>
          <p className="text-sm text-muted-foreground">Yıl {DEFAULT_YEAR} için aylık hedefleri girin.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Yıllık hedef toplamı</CardTitle>
        </CardHeader>
