...
export function ActualGrid({ managerId, year }: ActualGridProps) {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `/api/actuals/bulk?managerId=${managerId}&year=${year}`
      )
      if (response.ok) {
      ...
      toast({
        title: 'Hata',
        description: 'Gerçekleşmeler yüklenirken hata oluştu',
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: 'Başarılı',
        description: 'Gerçekleşmeler kaydedildi',
      })
      ...
      toast({
        title: 'Hata',
        description: 'Gerçekleşmeler kaydedilirken hata oluştu',
        variant: 'destructive',
      })
    } finally {
      setIsSaving(false)
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Aylık Gerçekleşmeler - {year}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {data.map((item) => (
            <div key={item.month} className="space-y-2">
              <Label htmlFor={`actual-${item.month}`}>
                {MONTH_NAMES_TR[item.month - 1]}
              </Label>
              <Input
                id={`actual-${item.month}`}
                type="number"
                min="0"
                step="0.01"
